...
def main():
    ...
    header, rounds, skips = load_ladder(args.ladder)
    if not rounds:
    ...
    parts = [
        ...
        "## 4. Verdict",
        "",
        args.verdict.strip() if args.verdict else "—",
        "",
        "## 5. Method & caveats",
        "",
        "\n".join(caveats),
        "",
        f"*Generated by bench_markdown_report.py on {today}. Source: `{os.path.basename(args.ladder)}`"
        + (f" + `{os.path.basename(args.textgen)}`" if args.textgen else "") + ".",
        "",
    ]
